...
class Aauth {
	...
	public function get_menu() {
		...
		/*if(!in_array($user_type, array(1, 2))){
			$this->CI->db->join('group_menu b', 'a.`ser_id`=b.`menu_id`');
			$this->CI->db->join('tbl_group c', 'b.`group_id`= c.`ser_id`');
			$this->CI->db->join('user_group d', 'c.`ser_id`=d.`group_id`');
			$this->CI->db->where('d.`user_id`', $user);
		}*/
		$this->CI->db->where('a.parent_menu', NULL);
